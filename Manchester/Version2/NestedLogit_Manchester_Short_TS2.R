...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  nlNests      = list(root=1, car = lambda_car, active=lambda_active)
  
  ### Specify tree structure for NL model
  nlStructure = list()
  nlStructure[["root"]]   = c("car","active","ptwalk")
...
model_s2 = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs,
estimate_settings=list(constraints=c(
                                      "lambda_car < 1 + 1e-10", "lambda_car > -1e-10",
                                      "lambda_active < 1 + 1e-10", "lambda_active > -1e-10")))
...
write.csv(forecast,file = "C:/Users/e18933/OneDrive - RMIT University/WORK/JIBE/DATA Analysis/STATA/ModeChoicePrediction.csv")

#### Likelihood ratio tests against MNL model
# ################################################################# #
lrTest = apollo_lrTest(modelsht2,modelmnl)                                                                                                                                                                                                                                                                                                                           
...
apollo_modelOutput(model_s2)

# Print estimates
library(dplyr)
model_s2_estimates <- model_s2$estimate
model_s2_tTest <- model_s2$estimate / model_s2$se
model_s2_pValue <- 2 * (1 - stats::pnorm(abs(model_s2$estimate/model_s2$se)))
model_s2_sig <- case_when(model_s2_pValue <= 0.001 ~ "***",
                       model_s2_pValue <= 0.01 ~ "**",
                       model_s2_pValue <= 0.05 ~ "*",
                       model_s2_pValue <= 0.1 ~ ".",
                       TRUE ~ "")
estimateValues <- paste(round(model_s2_estimates,3),"[",round(model_s2_tTest,3),model_s2_sig,"]")
estimateValues[model_estimates == 0] <- NA

write.csv(estimateValues,file = "C:/Users/e18933/OneDrive - RMIT University/WORK/JIBE/DATA Analysis/R/Manchester/estimatevalues_model_s2.csv")
